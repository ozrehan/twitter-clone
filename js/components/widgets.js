...
(function (App) {
  ...
  App.renderWidgets = function (root) {
    ...
    App.on(root, "click", "[data-follow]", async function (e, btn) {
      ...
      btn.disabled = true;
      try {
        var now = await App.toggleFollow(btn.getAttribute("data-follow"));
  ...
  function renderFollows(slot) {
    var sug = App.USERS.filter(function (u) {
      return u.username !== App.store.currentUserId && !App.isFollowing(u.username) && u.verified;
    }).slice(0, 3);
    slot.innerHTML = sug.map(function (u) {
      return '<div class="row">' +
        ...
        '<button class="follow-btn" data-follow="' + u.username + '">Follow</button></div>';
    }).join("");
  }
})(window.App);
